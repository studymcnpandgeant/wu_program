...
/*  Set neutron flux DM   */
  ...
  /* Output the data */
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu_test",FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
...
PetscErrorCode FormFunction(SNES snes,Vec U,Vec FU,void *dummy)
{
  ...
  for (j=yints; j<yinte; j++){
    for (i=xints; i<xinte; i++){
      ...
      // }
      /*phi1 field*/
      rightside = - 0.0121 * phi1[j][i] + lambda[0] * 0.0085 * phi1[j][i] ;//+ lambda[0] * 0.1851 * phi2[j][i] ;
...
/*TEST
   ...
   test:
      nsize: 1
      args: $PETSC_DIR/lib/petsc/bin/petscmpiexec -n 1 ./ex213 -f wu-read2 -snes_fd -pc_type lu -snes_view -snes_converged_reason -snes_monitor -ksp_converged_reason -ksp_monitor_true_residual
      note: not complete, differ from slepc
