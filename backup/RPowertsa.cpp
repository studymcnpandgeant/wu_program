...
void RPowertsa::setinitial(PetscScalar *array1,PetscScalar *array2,PetscScalar *array3,PetscScalar *array4,PetscScalar *array5,PetscScalar *array6,PetscScalar *array7,PetscScalar *array8,PetscScalar *array9)
{
    ...
    v[3] = 4.3e5;
    keff = 0.999883;
	dt = 1.0;//very important variable 
    beta[0]=4.9588439e-4;
	beta[1]=3.0678140e-3;
	beta[2]=2.6332880e-3;
	beta[3]=5.4992171e-3;
...
PetscScalar RPowertsa::gettsrs(PetscInt i, PetscInt j, PetscInt group, PetscScalar flux1, PetscScalar flux2, PetscScalar flux3, PetscScalar flux4, PetscScalar keff)
{
      ...
      for (i = 1; i < 45+1; i++)
      {
        for (gl = 1; gl < 5 ; gl++)
        {
          ...
          if (i>=25 && i<=28 && j>=1 && j<=87 && gl==4)
           {
           	CVvsiga[index3] = CVvsiga[index3] * 0.09058 + 0.0* time;  	
           } 
