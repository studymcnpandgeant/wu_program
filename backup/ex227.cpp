...
int main(int argc,char **argv)
{
  ...
  Vec            u101,u102,u103,u104,u105,u106,u107,u108,u109;//RP parameters
  ...
  user.settotalinitial(data.vv1,data.vv2,data.vv3,data.vv4,data.vv5,data.vv6,data.vv7,data.vv8,data.vv9,data.vv10,data.vv101,data.vv102,data.vv103,data.vv104,data.vv105,data.vv106,data.vv107,data.vv108,data.vv109);
  ...
  /* Output the data */
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu_mpcpout",FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
...
PetscErrorCode FormFunction(SNES snes,Vec U,Vec FU,void *dummy)
{
  ...
  for (j=1; j<104+1; j++){
    for (i=1; i<45+1; i++){
      ...
      keff       = u[Ilast];
      rightside = user->RPgetrs(i,j,group,phigroup1,phigroup2,phigroup3,phigroup4,keff) ;
      if ( i > 1 )
      {
        Ileft = II - 1 ;
        indexl = Ileft + (group-1)*45*104;
        ac1 = user->RPgeta1(i,j,group);
        cc1 = ac1 * u[indexl];
      }
      if ( i < 45 )
      {
      ...
      ac3 = user->RPgeta3(i,j,group);
      ac4 = user->RPgeta4(i,j,group);
      ac0 = user->RPgeta0(i,j,group,ac1,ac2,ac3,ac4);
  ...
  for (j = 1; j < 104+1; j++)
  {
    for (i = 1; i < 45+1; i++)
    {
      for (group = 1; group < 5 ; group++)
      {
        ...
        sigf = user->RPgetsigf(i,j,group);
        cvv   = user->RPgetcvv(i,j);
  ...
  {
    user->THP[i]  = u[i+ith];
    user->THTf[i] = u[i+THInterval1+ith];
    user->THU[i]  = u[i+THInterval2+ith];
    user->THV[i]  = u[i+THInterval3+ith];
  }
  for (i = 0; i < 4680 ; i++)
  {
    user->THTs[i] = u[i+THInterval4+ith];
  }
