...
extern PetscErrorCode FormJacobian(TS,PetscReal,Vec,Mat,Mat,void*);
...
int main(int argc,char **argv)
{
  ...
  ierr = TSSetRHSJacobian(ts,B,B,FormJacobian,&user);CHKERRQ(ierr);
  ...
  /* Output the data */
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu_mpcpout20190304",FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu_mpcpdflux20190304",FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
  ierr = PetscViewerPushFormat(viewer,PETSC_VIEWER_BINARY_MATLAB);CHKERRQ(ierr);
...
*/
PetscErrorCode FormFunction(TS ts,PetscReal t,Vec u,Vec FU,void *dummy)
{
  RPowerts     *user = (RPowerts*)dummy;
  PetscErrorCode ierr;
...
PetscErrorCode FormJacobian(TS ts,PetscReal t,Vec u,Mat J,Mat B,void *dummy)
{
  RPowerts       *user = (RPowerts*)dummy;
  PetscErrorCode ierr;
  PetscInt       xs,xm,i,N,II,n;
  ...
  for (j=1; j<105; j++){
    for (i=1; i<46; i++){
      ...
      if ( j < 104 )
      {
        ...
        a  = user->gettsa0(i,j,group,b,c,dd,e) ;
