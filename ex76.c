...
PetscErrorCode MatLModel(PetscInt n,PetscInt m,Mat L,void *ptr);
...
int main(int argc,char **argv)
{
  ...
  PetscReal      tol;
  Vec            xr,xi;
  Vec            u1,u2,u3,u4;    /* These vectors used for reading binary file */
  PetscViewer    viewer;
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu-sn2dtest3",FILE_MODE_READ,&viewer);CHKERRQ(ierr);
  ...
  ierr = VecLoad(u3,viewer);CHKERRQ(ierr);/* x-infomation */
  ierr = VecLoad(u4,viewer);CHKERRQ(ierr);/* y-infomation */
  ...
  ierr = MatLModel(n,m,L,&user);CHKERRQ(ierr);
...
PetscErrorCode MatLModel(PetscInt n, PetscInt m, Mat L,void *ptr)
{
  ...
  PetscReal       Dleft,Dright,Dup,Ddown;
  PetscReal       dxleft,dxright,dyup,dydown;
  PetscErrorCode  ierr;
