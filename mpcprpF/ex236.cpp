...
int main(int argc,char **argv)
{
  ...
  PetscErrorCode ierr;
  PetscInt       its;
  PetscInt       nx=45,ny=104,nglobal,nq;
  Vec            U,FU;
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"BCO",FILE_MODE_READ,&viewer);CHKERRQ(ierr);
  ierr = VecCreate(PETSC_COMM_WORLD,&u10);CHKERRQ(ierr);
  ierr = VecLoad(u10,viewer);CHKERRQ(ierr);/* materials data cross-section infomation */
  ierr = PetscViewerDestroy(&viewer);CHKERRQ(ierr);
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu_mpcpQf_0320_1",FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
  ierr = PetscViewerPushFormat(viewer,PETSC_VIEWER_BINARY_MATLAB);CHKERRQ(ierr);
...
{
  RPower2        *user = (RPower2*)dummy;
  PetscErrorCode ierr;
  PetscInt       xs,xm,i,II,n;
