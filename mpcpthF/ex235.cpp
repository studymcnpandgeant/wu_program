...
int main(int argc,char **argv)
{
  ...
  PetscErrorCode ierr;
  PetscInt       its;
  PetscInt       nx=22,ny=76,nglobal,nxts=45,nyts=104;
  Vec            U,FU;
  Vec            u1,u2,u3,u4,u5,u6,u7,u8,u9,u10;
  Vec            u21,u22,u23,u24,u25,u26,u27;
  ...
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu-THTsb",FILE_MODE_READ,&viewer);CHKERRQ(ierr);
  ...
  nglobal = 4 * nx * ny + nxts * nyts ;//set the global size of the solution vectors: five field.
  ...
  ierr = FormInitialGuess(U,u21,u22,u27,u24,u25);CHKERRQ(ierr);
  ...
  /* Output the data */
  ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,"wu_20190224",FILE_MODE_WRITE,&viewer);CHKERRQ(ierr);
...
PetscErrorCode FormInitialGuess(Vec U,Vec u21,Vec u22,Vec u27,Vec u24,Vec u25)
{
  PetscErrorCode ierr;
  PetscScalar    *u;
  ...
  for (j=1; j<104+1; j++){
    for (i=1; i<45+1; i++){
        II = (j-1)*45 + i - 1 ;
        u[II+Interval4] =  initial_THTs[II];    
    }
  }
...
PetscErrorCode FormJacobian(SNES snes,Vec U,Mat J,Mat B,void *dummy)
{
  ...
  PetscInt       i,II,n;
  PetscInt       j;/*y direction index*/
  PetscInt       Interval1,Interval2,Interval3,Interval4;
  PetscInt       Ileft,Iright,Idown,Iup;
  const PetscScalar    *u;
  ...
  for (j=12; j<87+1; j++){
    for (i=1; i<22+1; i++){
      ...
      /* U field */
      II = (j-12)*n + i - 1 + Interval2 ;
      ac0 = 1.0;
      ierr  = MatSetValues(B,1,&II,1,&II,&ac0,INSERT_VALUES);CHKERRQ(ierr);

      /* V field */
      II = (j-12)*n + i - 1 + Interval3 ;
      ac0 = 1.0;
      ierr  = MatSetValues(B,1,&II,1,&II,&ac0,INSERT_VALUES);CHKERRQ(ierr);
